...
- Field and Weather Effects (e.g. Rain Dance, Trick Room etc.) lasts for 8 turns (by default)
...
- Bodhisattva (Ground): 60 power, contact special, -2 priority, super-effective against flying type, increase SpDef by 1 stage
...
- History Rewritten (Normal): status, disable, reset opponent stat stages and confuse
- Soul Harvest (Dark): 40 power, special, +1 priority move, binding, drain 30% dmg
- Reign of Terror (Ghost): 90 power, special, 30% reduce target Spd and Acc by 1 stage, 30% frighten
- Empyrean Glory (Flying): status, buff user DEF and SpDef by 1 stage, heal team status condition problem, clear weather
- Draconic Blade (Dragon): 75 power, physical, 30% frighten
...
2. ability Illusion is partially useful to AI, due to Others#3.
