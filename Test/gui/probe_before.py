...
MW.MainWindow.ACTIONS_MAX_HEIGHT = 225   # pre-change cap
MW.MainWindow.ACTIONS_MIN_HEIGHT = 70
_ob = MW.MainWindow._build_body
def _build_body(self):
    row = _ob(self)
    row.setStretch(0, 3)                 # pre-change split
    return row
MW.MainWindow._build_body = _build_body
...
def report():
    ...
    with open(os.path.join(OUT, "geometry_before.txt"), "w", encoding="utf-8") as f:
        f.write(text + "\n")
