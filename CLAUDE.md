...
**Its results were deleted once, on purpose** -- they had been measured
against a move-ranking rule that no longer exists. `Documentation/ai_head_to_head.md`
has since been regenerated and now reports the AI as it stands: **55.7%
overall (+/-1.8), ahead in all five bands, and 56.7% in the weakest one**.
That last figure is the headline: the old study's central finding was that
the smart AI was a *liability* with weak teams at 42.6%, and that is gone.

Read the control before the results. Pooled over all bands it is 48.7%,
which is what makes the numbers readable; per band it swings between 37.8%
and 60.0% because each band is only 150 battles, so single-band figures are
noise. The orientation check is the better symmetry test and is balanced
(side A 56.0%, side B 55.3%).

Switching now runs at 7.2-7.7% of the smart side's turns, against the 12-15%
the earlier study recorded -- see STAGE_FORFEIT in ai.py, which credits
staying in with the stat stages a switch would throw away. Removing
switching entirely still moves the overall result by only -0.7 points, so
the earlier conclusion holds: switching is not what decides these bands.
